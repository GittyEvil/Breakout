...
public class PlayerBoard extends Sprite{
	...
	public PlayerBoard(int x, int y, int width, int height,Color color, int xBoundary, int yBoundary) {
	...
	public void update(Keyboard keyboard) {
		...
		int currentX = getX();
		if(keyboard.isKeyDown(Key.Right) && currentX < xBoundary - 100) {
			setX(getX() + this.speed);
		}
		if(keyboard.isKeyDown(Key.Left)&& currentX > 0) {
			setX(getX() - this.speed);
		}
